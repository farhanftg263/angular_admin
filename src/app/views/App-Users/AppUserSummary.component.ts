...
import { AlertService,AppUserService} from '../../_services';
import { FormGroup, FormControl, Validators,FormBuilder } from '@angular/forms';
import { stringify } from 'querystring';
@Component({
  selector: 'app-dashboard',
  templateUrl: 'AppUserSummary.component.html',
...
export class AppUserSummaryComponent implements OnInit  {
  ...
  users: FormGroup;
    ...
    */
    changeStatus(user)
    {
        if(confirm("Are you sure you want to change the status of this app user")) {
    ...
    */
   delete(id:string){
        if(confirm("Are you sure you want to delete this app user")) {
            console.log(id);
            this.user_id = id;
            if(!(this.user_id))
            {
    ...
    */
    searchedData(){
     ...
     /*
      Function Type : Resend Activation Email
      Author : Pradeep Chaurasia
      Created On : 09-10-2018
    */
    resendActivationEmail(id:string){
        if(confirm("Are you sure you want to resend activation email")) {
            ...
            // console.log('iiiiiiddddd '+this.cmsid); 
            this.userService.resendEmail(this.user_id).subscribe(
                data => {
                    this.userDeleteResponse = data;
                    if(this.userDeleteResponse.code == 200)
                    { 
